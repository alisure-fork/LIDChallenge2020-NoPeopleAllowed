...
class UNet(nn.Module):
    ...
    def __init__(self, encoder, num_classes):
        ...
        self.model = smp.Unet(
            encoder, classes=num_classes if num_classes != 2 else 1, encoder_weights="imagenet")
        pass
...
class MultiScaleDeeplabV3Resnet50(nn.Module):
    ...
    def __init__(self, num_classes, scales=(0.5, 0.75), pretrained=False, pretrained_backbone=True):
        ...
        self.model = deeplabv3_resnet50(
            num_classes=num_classes, pretrained=pretrained, pretrained_backbone=pretrained_backbone)
    ...
    def forward(self, x):
        ...
        out = torch.empty(size=(1 + len(self.scales), batch_size, self.num_classes, *shape), device=x.device)

        out[0] = self.model(x)["out"]
        for i, scale in enumerate(self.scales, start=1):
            x_scaled = F.interpolate(
                x, size=(int(shape[0] * scale), int(shape[1] * scale)), mode="bilinear", align_corners=False)
