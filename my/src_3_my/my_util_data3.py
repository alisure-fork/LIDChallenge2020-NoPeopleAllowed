...
class MyTransform(object):
    ...
    def transform_train_class(cls, input_size=224):
        ...
        transform_train = ExtCompose([ExtResize(size=input_size),
                                      ExtRandomCrop(size=input_size),
                                      ExtRandomHorizontalFlip(),
                                      ExtToTensor(),
                                      ExtNormalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
        transform_test = ExtCompose([ExtResize(size=input_size), ExtToTensor(),
                                     ExtNormalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
...
class VOCSegmentation(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.return_image_info:
            return image, mask, label, image_path, label_path
...
class DatasetUtil(object):
    ...
    @classmethod
    def get_dataset_by_type(cls, dataset_type, input_size, data_root=None,
