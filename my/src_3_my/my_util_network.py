...
class DualNet2(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.decoder = DualDecoder(num_classes=self.num_classes)
        pass
    ...
    def forward(self, x1, x2, pair_labels, has_class=False, has_cam=False, has_ss=False):
        ...
        cam_1 = self._cluster_activation_map(pair_labels, class_feature_1, self.decoder.class_l1.weight)  # 簇激活图
        cam_2 = self._cluster_activation_map(pair_labels, class_feature_2, self.decoder.class_l1.weight)  # 簇激活图
        cam_norm_1 = self._feature_norm(cam_1)
        cam_norm_2 = self._feature_norm(cam_2)
        ...
        if has_ss:
            ...
            d5_out_1 = self.decoder.decoder_5_out(d5_1)  # 21 * 160 * 160
            d5_out_softmax_1 = torch.softmax(d5_out_1, dim=0)  # 21 * 160 * 160  # 小输出
            d5_out_up_1 = self._up_to_target(d5_out_1, x1)  # 21 * 320 * 320
            d5_out_up_softmax_1 = torch.softmax(d5_out_up_1, dim=0)  # 21 * 320 * 320  # 大输出

            d5_out_2 = self.decoder.decoder_5_out(d5_2)  # 21 * 160 * 160
            d5_out_softmax_2 = torch.softmax(d5_out_2, dim=0)  # 21 * 160 * 160  # 小输出
            d5_out_up_2 = self._up_to_target(d5_out_2, x2)  # 21 * 320 * 320
            d5_out_up_softmax_2 = torch.softmax(d5_out_up_2, dim=0)  # 21 * 320 * 320  # 大输出
    ...
    def forward_inference(self, x, has_class=False, has_cam=False, has_ss=False):
        ...
        if has_class:
            ...
            if has_cam:
                _, top_k_index = torch.topk(class_logits, 1, 1)
                pred_labels = [int(one[0]) for one in top_k_index]
                cam = self._cluster_activation_map(pred_labels, class_feature, self.decoder.class_l1.weight)  # 簇激活图
        ...
        if has_ss:
            ...
            d5_out = self.decoder.decoder_5_out(d5)  # 21 * 160 * 160
            d5_out_softmax = torch.softmax(d5_out, dim=0)  # 21 * 160 * 160  # 小输出
            d5_out_up = self._up_to_target(d5_out, x)  # 21 * 320 * 320
            d5_out_up_softmax = torch.softmax(d5_out_up, dim=0)  # 21 * 320 * 320  # 大输出
    ...
    def _class_feature(self, feature):
        ...
        class_feature = self.decoder.class_c2(self.decoder.class_c1(e4))  # (512, 10)
        class_1x1 = F.adaptive_avg_pool2d(class_feature, output_size=(1, 1)).view((class_feature.size()[0], -1))
        class_logits = self.decoder.class_l1(class_1x1)
        return class_feature, class_logits

    def _convert_feature(self, feature):
        e0 = self.decoder.convert_1(feature["e0"])  # 128
        e1 = self.decoder.convert_2(feature["e1"])  # 256
        e2 = self.decoder.convert_3(feature["e2"])  # 256
        e3 = self.decoder.convert_4(feature["e3"])  # 512
        e4 = self.decoder.convert_5(feature["e4"])  # 512
        return e0, e1, e2, e3, e4

    def _encoder(self, e0, e1, e2, e3, e4):
        d1 = self.decoder.decoder_1_b2(self.decoder.decoder_1_b1(e4))  # 512 * 40 * 40
        d1_d2 = self._up_to_target(self.decoder.decoder_1_c(d1), e3) + e3  # 512 * 40 * 40

        d2 = self.decoder.decoder_2_b2(self.decoder.decoder_2_b1(d1_d2))  # 512 * 21 * 21
        d2_d3 = self._up_to_target(self.decoder.decoder_2_c(d2), e2) + e2  # 512 * 40 * 40

        d3 = self.decoder.decoder_3_b2(self.decoder.decoder_3_b1(d2_d3))  # 256 * 40 * 40
        d3_d4 = self._up_to_target(self.decoder.decoder_3_c(d3), e1) + e1  # 256 * 80 * 80

        d4 = self.decoder.decoder_4_b2(self.decoder.decoder_4_b1(d3_d4))  # 256 * 80 * 80
        d4_d5 = self._up_to_target(self.decoder.decoder_4_c(d4), e0) + e0  # 128 * 160 * 160

        d5 = self.decoder.decoder_5_b2(self.decoder.decoder_5_b1(d4_d5))  # 128 * 160 * 160
        return d5
