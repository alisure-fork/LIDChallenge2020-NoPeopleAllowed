...
class Backbone(nn.Module, ABC_Model):
    ...
    def __init__(self, model_name, num_classes=20, mode='fix', segmentation=False):
        super().__init__()
        self.mode = mode
        self.norm_fn = FixedBatchNorm if self.mode == 'fix' else nn.BatchNorm2d

        if 'resnet' in model_name:
            self.model = resnet.ResNet(resnet.Bottleneck, resnet.layers_dic[model_name],
                                       strides=(2, 2, 2, 1), batch_norm_fn=self.norm_fn)
            ...
            self.model = eval("resnest." + model_name)(
                pretrained=True, dilated=dilated, dilation=dilation, norm_layer=self.norm_fn)
...
class DeepLabv3_Plus(Backbone):
    ...
    def __init__(self, model_name, num_classes=21, mode='fix', use_group_norm=False):
        ...
        norm_fn_for_extra_modules = group_norm if use_group_norm else self.norm_fn
...
class CSeg_Model(Backbone):
    ...
    def __init__(self, model_name, num_classes=21):
        ...
        fc_edge1_features = 64 if '50' in model_name else 128
        self.fc_edge1 = nn.Sequential(nn.Conv2d(fc_edge1_features, 32, 1, bias=False),
                                      nn.GroupNorm(4, 32), nn.ReLU(inplace=True))
        self.fc_edge2 = nn.Sequential(nn.Conv2d(256, 32, 1, bias=False),
                                      nn.GroupNorm(4, 32), nn.ReLU(inplace=True))
        self.fc_edge3 = nn.Sequential(nn.Conv2d(512, 32, 1, bias=False), nn.GroupNorm(4, 32),
                                      nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
                                      nn.ReLU(inplace=True))
        self.fc_edge4 = nn.Sequential(nn.Conv2d(1024, 32, 1, bias=False), nn.GroupNorm(4, 32),
                                      nn.Upsample(scale_factor=4, mode='bilinear', align_corners=False),
                                      nn.ReLU(inplace=True))
        self.fc_edge5 = nn.Sequential(nn.Conv2d(2048, 32, 1, bias=False), nn.GroupNorm(4, 32),
                                      nn.Upsample(scale_factor=4, mode='bilinear', align_corners=False),
                                      nn.ReLU(inplace=True))
