...
parser.add_argument('--data_dir', default='/media/ubuntu/4T/ALISURE/Data/SS/voc/VOCdevkit/VOC2012/', type=str)
...
parser.add_argument('--tag', default='demo', type=str)
parser.add_argument('--label_name', default='demo', type=str)
...
if __name__ == '__main__':
    ...
    log_path = create_directory(f'./experiments/logs/') + f'{args.tag}.txt'
    data_path = create_directory(f'./experiments/data/') + f'{args.tag}.json'
    model_path = create_directory('./experiments/models/') + f'{args.tag}.pth'
    ...
    pred_dir = '/media/ubuntu/4T/ALISURE/Data/SS/voc/VOCdevkit/VOC2012/SegmentationClassAug/'
    ...
    train_transforms = [RandomResize_For_Segmentation(args.min_image_size, args.max_image_size),
    ...
    test_transform = transforms.Compose([Normalize_For_Segmentation(imagenet_mean, imagenet_std),
    ...
    if args.architecture == 'DeepLabv3+':
        model = DeepLabv3_Plus(args.backbone, num_classes=meta_dic['classes'] + 1,
                               mode=args.mode, use_group_norm=args.use_gn)
    ...
    params = [{'params': param_groups[0], 'lr': args.lr, 'weight_decay': args.wd},
    ...
    optimizer = PolyOptimizer(params, lr=args.lr, momentum=0.9, weight_decay=args.wd,
                              max_step=max_iteration, nesterov=args.nesterov)
    ...
    for iteration in range(max_iteration):
        ...
        if 'Seg' in args.architecture:
            labels = resize_for_tensors(labels.type(torch.FloatTensor).unsqueeze(1),
        ...
        if (iteration + 1) % log_iteration == 0:
            ...
            data = {'iteration' : iteration + 1, 'learning_rate' : learning_rate,
                    'loss' : loss, 'time' : train_timer.tok(clear=True)}
            data_dic['train'].append(data)
            write_json(data_path, data_dic)
            
            log_func('[i] iteration={iteration:,}, learning_rate={learning_rate:.4f}, '
                     'loss={loss:.4f}, time={time:.0f}sec'.format(**data))
        ...
        if (iteration + 1) % val_iteration == 0:
            ...
            data = {'iteration' : iteration + 1, 'mIoU' : mIoU,
                    'best_valid_mIoU' : best_valid_mIoU, 'time' : eval_timer.tok(clear=True)}
            data_dic['validation'].append(data)
            write_json(data_path, data_dic)
            
            log_func('[i] iteration={iteration:,}, mIoU={mIoU:.2f}%, '
